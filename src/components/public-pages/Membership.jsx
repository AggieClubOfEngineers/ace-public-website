import React from "react";
import { BrandLinkedin } from "tabler-icons-react";

import "../../styles/membership.css";
import Grant_Schoettlekotte from "../../assets/img/2022Headshots/grantShuttle.jpg";
import Joey_Kacer from "../../assets/img/2022Headshots/joeyKacer.jpg";
import Lucas_Giammona from "../../assets/img/2022Headshots/lucasGiammona.jpg";
import Matthew_Oakland from "../../assets/img/2022Headshots/mattOakland.jpg";
import Nate_Mathews from "../../assets/img/2022Headshots/nateMatthew.jpg";
import Tyler_Powell from "../../assets/img/2022Headshots/tylerPowell.jpg";
import Nick_Dittemore from "../../assets/img/2023Headshots/Nick_Dittemore.jpg";
import Brandon_Turnage from "../../assets/img/2023Headshots/Brandon_Turnage.JPG";
import Connor_Gilliland from "../../assets/img/2023Headshots/Connor_Gilliland.JPG";
import Jonathan_Kutsch from "../../assets/img/2023Headshots/Jon_Kutsch.JPG";
import Drew_Remington from "../../assets/img/2023Headshots/Drew_Remington.JPG";
import Amrit_Nanda from "../../assets/img/2023Headshots/Amrit_Nanda.JPG";
import Andrew_Lin from "../../assets/img/2023Headshots/Andrew_Lin.JPG";
import Jacob_Technik from "../../assets/img/2023Headshots/Jacob_Technik.JPG";
import Jess_Holbert from "../../assets/img/2023Headshots/Jess_Holbert.JPG";
import Joseph_Valenta from "../../assets/img/2023Headshots/Joseph_Valenta.JPG";
import Joshua_Mayhugh from "../../assets/img/2023Headshots/Joshua_Mayhugh.JPG";
import Jack_Bridges from "../../assets/img/2023Headshots/Jack_Bridges.JPG";
import Preston_Greenwood from "../../assets/img/2023Headshots/Preston_Greenwood.JPG";
import Sid_Venkatraman from "../../assets/img/2023Headshots/Sid_Venkatraman.jpeg";
import Andrew_Marshall from "../../assets/img/2023Headshots/Andrew_Marshall.JPG";
import Hudson_Hurtig from "../../assets/img/2023Headshots/Hudson_Hurtig.JPG";
import Nils_Ljung from "../../assets/img/2023Headshots/Nils_Ljung.JPG";
import Peyton_Woytek from "../../assets/img/2023Headshots/Peyton_Woytek.JPG";
import Ryan_Riddle from "../../assets/img/2023Headshots/Ryan_Riddle.JPG";
import Van_Wilcox from "../../assets/img/2023Headshots/Van_Wilcox.JPG";
import Vinny_LoBello from "../../assets/img/2023Headshots/Vinny_LoBello.JPG";
import Whitten_Bowles from "../../assets/img/2023Headshots/Whitten_Bowles.JPG";
import Scott_Trouy from "../../assets/img/2023Headshots/Scott_Trouy.JPG";
import Eddie_Villegas from "../../assets/img/2023Headshots/Eddie_Villegas.JPG";
// non-executive staff
import Anthony_Salazar from "../../assets/img/2023Headshots/Anthony_Salazar.JPG";
import Justen_Pearl from "../../assets/img/2022Headshots/justenPearl.jpg";
import Juan_Nerio from "../../assets/img/2023Headshots/Juan_Nerio.JPG";
import Kyle_Dessens from "../../assets/img/2022Headshots/kyleDessens.jpg";
import Adrian_Guzman from "../../assets/img/2023Headshots/Adrian_Guzman.JPG";
import Brandon_Zhao from "../../assets/img/2023Headshots/Brandon_Zhao.JPG";
import Ankit_Lulla from "../../assets/img/2023Headshots/Ankit_Lulla.JPG";
import Nathan_Weiblen from "../../assets/img/2023Headshots/Nathan_Weiblen.JPG";
import Zach_Iacobucci from "../../assets/img/2023Headshots/Zach Iacobucci.JPG";
import Lucas_Swoyer from "../../assets/img/2023Headshots/Lucas_Swoyer.JPG";
import Peyton_Knight from "../../assets/img/2023Headshots/Peyton_Knight.JPG";

import Micaiah_Wood from "../../assets/img/2023Headshots/Micaiah_Wood.JPG";
import Juan_Ardila from "../../assets/img/2023Headshots/Juan_Ardila.JPG";
import Nick_Mozyrsky from "../../assets/img/2023Headshots/Nick_Mozyrsky.JPG";
import Aidan_Pena from "../../assets/img/2022Headshots/aidenPena.jpg";
import Chad_Cordova from "../../assets/img/2023Headshots/Chad_Cordova.JPG";
import Matthew_Graham from "../../assets/img/2022Headshots/matthewGraham.JPG";
import Jackson_David from "../../assets/img/2023Headshots/Jackson_David.JPG";
import Damon_Banduk from "../../assets/img/2023Headshots/Damon_Banduk.JPG";
import Nic_Rogstad from "../../assets/img/2021Headshots/rogstadhedshot.jpg";
import Mark_Thiele from "../../assets/img/2023Headshots/Mark_Thiele.JPG";
import Nick_Caso from "../../assets/img/2023Headshots/Nick_Caso.JPG";
import Chris_Choucair from "../../assets/img/2023Headshots/Chris_Choucair.JPG";
import Reed_Huffmyer from "../../assets/img/2023Headshots/Reed_Huffmyer.JPG";
import Alan_Marini from "../../assets/img/2023Headshots/Alan_Marini.JPG";
import Luke_Sciba from "../../assets/img/2022Headshots/lukeSciba.JPG";
import Dylan_Bell from "../../assets/img/2022Headshots/dylanBell.JPG";
import Parker_Briney from "../../assets/img/2023Headshots/Parker_Briney.JPG";
import Eric_McGonagle from "../../assets/img/2022Headshots/ericMcGon.jpg";
import Luis_Calvo_Gerstner from "../../assets/img/2023Headshots/Luis_Calvo.JPG";
import Alejandro_Velasco from "../../assets/img/2023Headshots/Alejandro_Velasco.JPG";
import Erick_Lemus from "../../assets/img/2022Headshots/erickLemus.JPG";
import Calvin_Turrell from "../../assets/img/2023Headshots/Calvin_Turrell.JPG";
import Hunter_Aschen from "../../assets/img/2022Headshots/hunterAschen.jpg";
import Jackson_Owen from "../../assets/img/2023Headshots/Jackson_Owen.JPG";
import Dakota_Fathke from "../../assets/img/2022Headshots/dakotaFaethke.jpg";
import Nick_Moreno from "../../assets/img/2021Headshots/morenoheadshot.jpg";
import Julian_Pollina from "../../assets/img/2023Headshots/Julian_Pollina.JPG";
import Jon_Kutsch from "../../assets/img/2023Headshots/Jon_Kutsch.JPG";
import Connor_Dunn from "../../assets/img/2023Headshots/Connor_Dunn.JPG";
import Aidan_King from "../../assets/img/2023Headshots/Aidan_King.JPG";
import Ryan_Ozelton from "../../assets/img/2023Headshots/Ryan_Ozelton.JPG";
import John_Hayes from "../../assets/img/2023Headshots/John_Hayes.JPG";
import Grant_Ward from "../../assets/img/2023Headshots/Grant_Ward.JPG";
import Zach_Podraza from "../../assets/img/2023Headshots/Zach_Podraza.JPG";
import Gerardo_Ruiz from "../../assets/img/2022Headshots/gerryRuiz.jpg";
import Ben_White from "../../assets/img/2022Headshots/benWhite.jpg";
import Nick_Bodenheimer from "../../assets/img/2022Headshots/nickBod.jpg";
import Landon_Miller from "../../assets/img/2023Headshots/Landon_Miller.JPG";
import Chinemerem_Obi from "../../assets/img/2023Headshots/Chinemerem_Obi.JPG";
import Ben_Snavely from "../../assets/img/2023Headshots/Ben_Snavely.JPG";
import Jaden_Banze from "../../assets/img/2023Headshots/Jaden_Blaze.JPG";
import Jacob_Hargreaves from "../../assets/img/2023Headshots/Jacob_Hargreaves.JPG";
import Chase_Albright from "../../assets/img/2022Headshots/chaseAlbright.JPG";
import Mason_Joyner from "../../assets/img/2023Headshots/Mason_Joyner.JPG";
import Jake_Segers from "../../assets/img/2023Headshots/Jake_Segars.JPG";
import Manuel_PanDavila from "../../assets/img/2023Headshots/Manuel_PanDavila.JPG";
import Ben_Brod from "../../assets/img/2022Headshots/benBrod.JPG";
const Membership = () => (
  <div className="membership-content">
    {peopleByCategory.map((category, index, array) => (
      <section
        key={category.title}
        className={`category-container ${
          index === array.length - 1
            ? "last-category "
            : "" + index == 0
            ? "first-category"
            : ""
        }`}
      >
        <h2>{category.title}</h2>
        <div className="people-container">
          {category.people.map((person) => (
            <div
              key={person.name}
              className={
                "person-container" + (person.position ? "" : " active")
              }
            >
              {person.linkedin && (
                <a href={person.linkedin} className="linkedin-link">
                  <BrandLinkedin
                    className="person-linkedin"
                    size={50}
                    strokeWidth={1.5}
                    color="white"
                  />
                </a>
              )}
              <div
                className={
                  "person-info-top" + (person.position ? "" : " active")
                }
              >
                {person.position && (
                  <h3 className="person-position">{person.position}</h3>
                )}
              </div>
              <img
                src={person.image}
                alt={person.name}
                className="person-image"
              />
              <div className="person-info-bottom">
                <h2 className="person-name">{person.name}</h2>
                {person.major && <h3 className="person-major">{person.major}</h3>}
             
              </div>
            </div>
          ))}
        </div>
      </section>
    ))}
  </div>
);

const peopleByCategory = [
  {
    title: "Executive Staff",
    people: [
      {
        name: "Brandon Turnage",
        position: "President",
        linkedin: "https://www.linkedin.com/in/brandon-turnage-a5ab8222b/",
        major: "Civil Engineering '25",
        image: Brandon_Turnage,
      },

      {
        name: "Jess Holbert",
        position: "Internal VP",
        linkedin: "https://www.linkedin.com/in/jess-holbert-071b19161/",
        major: "Mechanical Engineering '24",
        image: Jess_Holbert,
      },
      {
        name: "Jacob Technik",
        position: "External VP",
        linkedin: "https://www.linkedin.com/in/jacob-technik-565a18239/",
        major: "Industrial & Systems '26",
        image: Jacob_Technik,
      },
      {
        name: "Jack Bridges",
        position: "Secretary",
        linkedin: "https://www.linkedin.com/in/jack-bridges-69b107246/",
        major: "Industrial Distribution '25",
        image: Jack_Bridges,
      },
      {
        name: "John Hayes",
        position: "Treasurer",
        linkedin: "https://www.linkedin.com/in/john-hayes-547403256/",
        major: "Industrial Engineering'26 & MS Finance '27",
        image: John_Hayes,
      },
      {
        name: "Zach Podraza",
        position: "Member at Large",
        linkedin: "https://www.linkedin.com/in/zachary-podraza-9b318620b/",
        major: "Industrial Distribution '24",
        image: Zach_Podraza,
      },
    ],
  },
  {
    title: "Internal Cabinet",
    people: [
      {
        name: "Mason Joyner",
        position: "Academic Chair",
        linkedin: "https://www.linkedin.com/in/mason-joyner/",
        major: "Industrial Engineering '26",
        image: Mason_Joyner,
      },
      {
        name: "Julian Pollina",
        position: "Athletics Chair",
        linkedin: "https://www.linkedin.com/in/julian-pollina-71b2902b1/",
        major: "Industrial Distribution '26",
        image: Julian_Pollina,
      },
      {
        name: "Alan Marini",
        position: "Family Chair",
        linkedin: "https://www.linkedin.com/in/alan-marini-a42a6b283/",
        major: "Electrical Engineering '26",
        image: Alan_Marini,
      },
      {
        name: "Andrew Marshall",
        position: "Judicial Chair",
        linkedin: "https://www.linkedin.com/in/andrew-marshall-559566256/",
        major: "Aerospace Engineering '26",
        image: Andrew_Marshall,
      },
      {
        name: "Landon Miller",
        position: "Philanthropy Chair",
        linkedin: "",
        major: "Mechanical Engineering '27",
        image: Landon_Miller,
      },
      {
        name: "Nick Caso",
        position: "Recruitment Chair",
        linkedin: "https://www.linkedin.com/in/nicholas-caso/",
        major: "Electrical Engineering & MS Finance '24",
        image: Nick_Caso,
      },
      {
        name: "Nate Mathews",
        position: "Recruitment Chair",
        linkedin: "https://www.linkedin.com/in/nathanielmathews/",
        major: "Industrial Engineering '24",
        image: Nate_Mathews,
      },
      {
        name: "Brandon Zhao",
        position: "Service Chair",
        linkedin: "https://www.linkedin.com/in/brandonzhao13/",
        major: "Biomedical Engineering '26",
        image: Brandon_Zhao,
      },
      {
        name: "Eric McGonagle",
        position: "Webmaster",
        linkedin: "https://www.linkedin.com/in/eric-mcgonagle/",
        major: "Computer Science '25",
        image: Eric_McGonagle,
      },
    ],
  },
  {
    title: "External Cabinet",
    people: [
      {
        name: "Juan Nerio",
        position: "Alumni Chair",
        linkedin: "https://www.linkedin.com/in/juannerioiii/",
        major: "Civil Engineering",
        image: Juan_Nerio,
      },
      {
        name: "Eddie Villegas",
        position: "Corporate Chair",
        linkedin: "https://www.linkedin.com/in/eddie-villegas-29431b253/",
        major: "Electrical Engineering '25",
        image: Eddie_Villegas,
      },
      {
        name: "Jacob Hargreaves",
        position: "Public Relations Chair",
        linkedin: "https://www.linkedin.com/in/jacob-hargreaves-89035028a/",
        major: "Mechanical Engineering '26",
        image: Jacob_Hargreaves,
      },
      {
        name: "Reed Huffmyer",
        position: "Quartermaster",
        linkedin: "https://www.linkedin.com/in/reedhuffmyer2023/",
        major: "Petroleum Engineering '25",
        image: Reed_Huffmyer,
      },
      {
        name: "Lucas Giammona",
        position: "Social Chair",
        linkedin: "https://www.linkedin.com/in/lucas-giammona-340457230/",
        major: "Mechanical Engineering '25",
        image: Lucas_Giammona,
      },
      {
        name: "Manuel PanDavila",
        position: "Social Media Chair",
        linkedin: "https://www.linkedin.com/in/manuel-j-pandavila-505879274/",
        major: "Materials Science & Engr '25",
        image: Manuel_PanDavila,
      },
      {
        name: "Connor Dunn",
        position: "Songfest Chair",
        linkedin: "https://www.linkedin.com/in/connordunn11/",
        major: "Biomedical Engineering '26",
        image: Connor_Dunn,
      },
      {
        name: "Preston Greenwood",
        position: "Special Events Chair",
        linkedin: "https://www.linkedin.com/in/preston-j-greenwood/",
        major: "Multidisc Engineering Tech '25",
        image: Preston_Greenwood,
      },
      {
        name: "Jackson David",
        position: "Tailgate Chair",
        linkedin: "",
        major: "Chemical Engineering '25",
        image: Jackson_David,
      },
    ],
  },
  {
    title: "Godfathers",
    people: [
      {
        name: "Justen Pearl",
        position: "Zapdos Godfather",
        linkedin: "https://www.linkedin.com/in/justen-pearl-2b9706263/",
        major: "Multidisc Engineering Tech '24",
        image: Justen_Pearl,
      },
      {
        name: "Hudson Hurtig",
        position: "Zapdos Godfather",
        linkedin: "https://www.linkedin.com/in/hudson-hurtig-2143a51ab/",
        major: "Interdisciplinary Engineering '26",
        image: Hudson_Hurtig,
      },
      {
        name: "Van Wilcox",
        position: "ISEN Godfather",
        linkedin: "https://www.linkedin.com/in/van-wilcox-a3954b290/",
        major: "Industrial Distribution '25",
        image: Van_Wilcox,
      },
      {
        name: "Juan Ardila",
        position: "ISEN Godfather",
        linkedin: "https://www.linkedin.com/in/juancardila/",
        major: "Industrial Engineering '25",
        image: Juan_Ardila,
      },
      {
        name: "Joseph Valenta",
        position: "MEEN Godfather",
        linkedin: "",
        major: "Mechanical Engineering '25",
        image: Joseph_Valenta,
      },
      {
        name: "Kyle Dessens",
        position: "MEEN Godfather",
        linkedin: "https://www.linkedin.com/in/kyle-dessens-0a91b1293/",
        major: "Mechanical Engineering '25",
        image: Kyle_Dessens,
      },
      {
        name: "Nick Bodenheimer",
        position: "PETE Godfather",
        linkedin: "https://www.linkedin.com/in/nick-bodenheimer-b542a2284/",
        major: "Petroleum Engineering '26",
        image: Nick_Bodenheimer,
      },
      {
        name: "Alejandro Velasco",
        position: "PETE Godfather",
        linkedin: "https://www.linkedin.com/in/alejandro-velasco-861865239/",
        major: "Industrial Distribution '26",
        image: Alejandro_Velasco,
      },
    ],
  },
  // {
  //   title: "Active Members",
  //   people: [
  //     {
  //       name: "Parker Briney",
  //       image: Parker_Briney,
  //     },
  //     {
  //       name: "Nick Mozyrsky",
  //       image: Nick_Mozyrsky,
  //     },
  //     {
  //       name: "Dakota Fathke",
  //       image: Dakota_Fathke,
  //     },
  //     {
  //       name: "Matthew Graham",
  //       image: Matthew_Graham,
  //     },
  //     {
  //       name: "Ben White",
  //       image: Ben_White,
  //     },
  //     {
  //       name: "Justen Pearl",
  //       image: Justen_Pearl,
  //     },
  //     {
  //       name: "Chinemerem Obi",
  //       image: Chinemerem_Obi,
  //     },
  //     {
  //       name: "Chris Choucair",
  //       image: Chris_Choucair,
  //     },
  //     {
  //       name: "Calvin Turrell",
  //       image: Calvin_Turrell,
  //     },
  //     {
  //       name: "Alejandro Velasco",
  //       image: Alejandro_Velasco,
  //     },
  //     {
  //       name: "Grant Ward",
  //       image: Grant_Ward,
  //     },
  //     {
  //       name: "Matthew Oakland",
  //       image: Matthew_Oakland,
  //     },
  //     {
  //       name: "Ben Snavely",
  //       image: Ben_Snavely,
  //     },
  //     {
  //       name: "Anthony Salazar",
  //       image: Anthony_Salazar,
  //     },
  //     {
  //       name: "Peyton Knight",
  //       image: Peyton_Knight,
  //     },
  //     {
  //       name: "Jon Kutsch",
  //       image: Jon_Kutsch,
  //     },
  //     {
  //       name: "Zach Iacobucci",
  //       image: Zach_Iacobucci,
  //     },
  //     {
  //       name: "Dylan Bell",
  //       image: Dylan_Bell,
  //     },
  //     {
  //       name: "Micaiah Wood",
  //       image: Micaiah_Wood,
  //     },
  //     {
  //       name: "Luke Sciba",
  //       image: Luke_Sciba,
  //     },
  //     {
  //       name: "Adrian Guzman",
  //       image: Adrian_Guzman,
  //     },
  //     {
  //       name: "Ankit Lulla",
  //       image: Ankit_Lulla,
  //     },
  //     {
  //       name: "Chad Cordova",
  //       image: Chad_Cordova,
  //     },
  //     {
  //       name: "Juan Ardila",
  //       image: Juan_Ardila,
  //     },
  //     {
  //       name: "Nathan Weiblen",
  //       image: Nathan_Weiblen,
  //     },
  //     {
  //       name: "Kyle Dessens",
  //       image: Kyle_Dessens,
  //     },
  //     {
  //       name: "Ryan Ozelton",
  //       image: Ryan_Ozelton,
  //     },
  //     {
  //       name: "Luis Calvo Gerstner",
  //       image: Luis_Calvo_Gerstner,
  //     },
  //     {
  //       name: "Damon Banduk",
  //       image: Damon_Banduk,
  //     },
  //     {
  //       name: "Scott Trouy",
  //       image: Scott_Trouy,
  //     },

  //     {
  //       name: "Hunter Aschen",
  //       image: Hunter_Aschen,
  //     },
  //     {
  //       name: "Jake Segers",
  //       image: Jake_Segers,
  //     },
  //     {
  //       name: "Whitten Bowles",
  //       image: Whitten_Bowles,
  //     },
  //     {
  //       name: "Aidan Pena",
  //       image: Aidan_Pena,
  //     },
  //     {
  //       name: "Chase Albright",
  //       image: Chase_Albright,
  //     },
  //     {
  //       name: "Mark Thiele",
  //       image: Mark_Thiele,
  //     },
  //     {
  //       name: "Gerardo Ruiz",
  //       image: Gerardo_Ruiz,
  //     },
  //     {
  //       name: "Jackson Owen",
  //       image: Jackson_Owen,
  //     },
  //     {
  //       name: "Aidan King",
  //       image: Aidan_King,
  //     },
  //     {
  //       name: "Nick Bodenheimer",
  //       image: Nick_Bodenheimer,
  //     },
  //     {
  //       name: "Hudson Hurtig",
  //       image: Hudson_Hurtig,
  //     },
  //     {
  //       name: "Andrew Lin",
  //       image: Andrew_Lin,
  //     },
  //     {
  //       name: "Joseph Valenta",
  //       image: Joseph_Valenta,
  //     },
  //     {
  //       name: "Joshua Mayhugh",
  //       image: Joshua_Mayhugh,
  //     },
  //     {
  //       name: "Peyton Woytek",
  //       image: Peyton_Woytek,
  //     },
  //     {
  //       name: "Van Wilcox",
  //       image: Van_Wilcox,
  //     }
  //   ],
  // },
];


export default Membership;
